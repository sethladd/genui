# `flutter_genui` Package Context for AI Agents

## Purpose

This document provides context for AI agents working with the `flutter_genui` package. This package is the core framework for building generative user interfaces in Flutter, where the UI is dynamically constructed by an AI model in real-time.
...
- **`ContentGenerator`**: An abstract interface for communicating with AI models. Concrete implementations (like `FirebaseAiContentGenerator` in `flutter_genui_firebase_ai`) handle specific model APIs.
...
- `lib/flutter_genui.dart`: Main export file.
